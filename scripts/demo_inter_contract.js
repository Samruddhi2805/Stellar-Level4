...
async function demonstrateFeeCalculation() {
    ...
    try {
        ...
        const applicableTier = feeTiers.find(tier => 
            parseInt(amount) >= tier.min && parseInt(amount) <= tier.max
        );
        
        if (applicableTier) {
            const baseFee = Math.floor((parseInt(amount) * applicableTier.rate) / 10000);
 ...
 */
async function demonstrateRewardCalculation(amount, user) {
    console.log('\n🎁 Step 2: Reward Contract - Cashback Calculation');
    console.log('-----------------------------------------------');
    
    try {
        console.log(`User: ${user.slice(0, 12)}...`);
        console.log(`Transaction Amount: ${StellarSdk.xlmToStroops(amount) / 10000000} XLM`);
        
        // Cashback calculation logic (from reward contract)
        const cashbackRate = 100; // 1% in basis points
        const cashbackAmount = Math.floor((parseInt(amount) * cashbackRate) / 10000);
        ...
        console.error('Cashback calculation error:', error);
    }
 ...
 */
async function demonstrateSplitProcessing(totalAmount, recipients) {
    console.log('\n👥 Step 3: Split Contract - Multi-Recipient Processing');
    console.log('----------------------------------------------------');
    
    try {
        console.log(`Total Amount: ${StellarSdk.xlmToStroops(totalAmount) / 10000000} XLM`);
        console.log(`Recipients: ${recipients.length}`);
        
        // Split contract logic
        const splitFeeRate = 50; // 0.5% fee
        const splitFee = Math.floor((parseInt(totalAmount) * splitFeeRate) / 10000);
        const distributableAmount = parseInt(totalAmount) - splitFee;
    ...
    return { splitRecipients: [], splitFee: 0 };
}

/**
 * Step 4: Demonstrate Complete Inter-Contract Flow
 */
async function demonstrateInterContractFlow() {
    ...
    try {
        ...
        console.log(`Original Amount: ${StellarSdk.xlmToStroops(transactionAmount) / 10000000} XLM`);
        console.log(`Sender: ${accounts.sender.slice(0, 12)}...`);
        
        // Step 1: Payment Contract calls Fee Contract
        console.log('\n📞 Payment Contract → Fee Contract');
        const feeAmount = await demonstrateFeeCalculation();
        
        // Step 2: Payment Contract calls Reward Contract
        console.log('\n📞 Payment Contract → Reward Contract');
        const cashbackAmount = await demonstrateRewardCalculation(transactionAmount, accounts.sender);
        ...
        const { splitRecipients, splitFee } = await demonstrateSplitProcessing(transactionAmount, recipients);
        ...
        return {
            transactionId,
            originalAmount: parseInt(transactionAmount),
            feeAmount,
            splitFee,
            cashbackAmount,
            totalFees,
        ...
        console.error('Inter-contract flow error:', error);
    }
}
...
async function runDemo() {
    ...
    try {
        ...
        const transactionData = await demonstrateInterContractFlow();
